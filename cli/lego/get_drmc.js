 ...
 * @type {string}
 */
cli.usage = 'edp lego get_drmc [--by=(mcid|mid)] [--format=(json|file|js)] '
    + '[--output_dir=<dir>] [<id_in_file>] [<ID>[,<ID>]]';
    ...
    function getIds(idFile, ids) {
        ...
        if (idFile) {
            if (fs.existsSync(idFile)) {
                var idStr = fs.readFileSync(idFile, 'utf-8');
                try {
                    // 先尝试JSON解析
                    var idsInFile = JSON.parse(idStr);
